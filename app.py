...
# Derived properties
density0 = row["density_ppg"]; pv0 = row["plastic_viscosity_cP"]; yp = row["yield_point_lb100ft2"]
...
# MAIN HEADER (LOGO)
st.image("logo.png", width=140)   # ← LOGO header
...
with col1:
    if "ECD" in show:
        ...
        z = depth_arr
        safe_mask = fracture_grad > pore_press
        if safe_mask:
            ax.fill_betweenx(z, pore_press, fracture_grad, color="#0b5", alpha=0.1)
...
with col2:
    ...
    if "Schematic" in show:
        ...
        ax4.fill_betweenx([0, depth], 0.2, 2.8, color="#0a0f12")
        ...
        ax4.text(2.9, toc, f"TOC {int(toc)} ft", color="white")
        ax4.text(2.9, depth, f"TD {int(depth)} ft", color="white")
        ax4.axis("off")
        st.pyplot(fig4)
